...
layui.use(['layuiRequest','form','layer','laydate','table','laytpl'],function(){
    ...
    var collectionRobotListIns =  table.render({
        ...
        cols : [[
            ...
            {field: 'result', title: '运行状态', minWidth:100, templet: '#timerResult', align:'center'},
            {field: 'resDate', title: '最近运行时间', minWidth:100, align:'center'},
    ...
    form.on('switch(timerStatus)', function(data) {
        ...
        var url = ctx + '/timer/common/stopJob?jobName=' + this.value;
        if(data.elem.checked) {
            tipText = '确定启用当前任务？';
            url = ctx + '/timer/common/startJob?jobName=' + this.value;
        }
    ...
    function editTimer(data) {
    	var index = layui.layer.open({
            ...
            moveType: 1,
            content: [ctx + '/timer/common/editTimer?jobName=' + data.jobName],
    ...
    function delTimer(data) {
        layer.msg('确定删除此任务？', {
        	...
        	btn: ['确定', '取消'],
            btn1: function(index, layero) {
            	layuiRequest.delTimer(ctx + '/timer/common/deleteJob', data, index, collectionRobotListIns);
            },
            btn2: function(index, layero) {
            	layer.close(index);
            }
    ...
    $('#addTimer').click(function () {
    	var addJobIndex = layui.layer.open({
            ...
            moveType: 1,
            content: [ctx + '/timer/common/addTimer'],
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                setTimeout(function(){
                    layui.layer.tips('点击此处返回任务列表', '.layui-layer-setwin .layui-layer-close', {
